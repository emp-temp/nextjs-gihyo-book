...
/**
 * テキスト
 * バリアント、色、タイポグラフィー、レイアウト、スペース関連のPropsを追加
 */
const Text = styled.span<TextProps>`
  ${({ variant, fontSize, letterSpacing, lineHeight, theme }) => {
    ...
    if (variant && variants[variant]) {
      ...
      !fontSize &&
        styles.push(
          toPropsValue('font-size', variants[variant].fontSize, theme)
        );
      !letterSpacing &&
        styles.push(
          toPropsValue('letter-spacing', variants[variant].letterSpacing, theme)
        );
      !lineHeight &&
        styles.push(
          toPropsValue('line-height', variants[variant].lineHeight, theme)
        );
